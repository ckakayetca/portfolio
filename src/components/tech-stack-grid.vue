<template>
    <div class="grid grid-cols-3 gap-7 w-full">
        <a v-for="tech in techStack" :key="tech.name" :href="tech.link" target="_blank" :class="`flex rounded-md bg-white/10 dark:bg-neutral-700/80 flex-col items-center justify-center gap-8 px-5 py-7 transition-all duration-200 border-neutral-400 border dark:border-neutral-600`" @mouseover="(event) => onHover(event, tech.color)" @mouseleave="onLeave">
            <img :src="tech.image" :alt="tech.name" class="w-32 h-32 transition-all duration-100" />
